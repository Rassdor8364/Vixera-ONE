...
| Manifest `VITE_SUPABASE_URL` equals `SUPABASE_PROJECT_URL` in `release-identity.env`; tree was clean (`VIXERA_ALLOW_DIRTY=1` for a local trial); every artifact of the release was built from one commit | a build baked from a dev `.env` used to pass with any 20-character project ref; a dirty tree is not reproducible from any commit; a leftover artifact of the same version from an earlier build would be re-summed and blessed |
